...
// @version			0.4.7
// @author			yLark, asmodai27, artorias, amal, sheychen
...
// @downloadURL   https://github.com/sheychen290/leekwars/raw/master/UserScripts/AdvancedTooltips.user.js
// @updateURL   https://github.com/sheychen290/leekwars/raw/master/UserScripts/AdvancedTooltips.user.js
...
function match_test(self) {
	...
	if (!/menu|menu-wrapper|menu-button|loading|game|tabs/i.test(self.parentNode.id) &&
		...
		if (/^(?:https:\/\/(beta.)?leekwars.com)?\/(leek|fight|report|farmer)\/([0-9]+)$/i.test(
                    self.getAttributeNS("https://www.w3.org/1999/xlink", "href"))) {
			var link_type = ((RegExp.$2 == 'report') ? 'fight' : RegExp.$2);
			return {
				type: link_type,
				id: RegExp.$3
...
function fill_leek(tooltip, target, $data) {
	...
	leek.innerHTML = '<a title="Poireau" href="/leek/' + target.id + '">' + leek_name + '</a><a href="/garden/challenge/' + target.id + '"><img src="https://leekwars.com/static//image/icon/garden.png" alt="Defier" style="height: 1em; filter: invert(1);"/></a>';
